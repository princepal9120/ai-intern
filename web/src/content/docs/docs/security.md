...
No real credential ever enters a container. Egress is intercepted in the Worker (`src/egress.ts`), where the real key is swapped in:
...
Local implementation: `src/index.ts`, `src/egress.ts`, `src/sandbox.ts`, `src/security.ts`, `src/automations.ts`, and `src/agents/orchestrator.ts`. See [Readiness](/docs/readiness/).
